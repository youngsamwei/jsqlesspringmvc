...
<script type="text/javascript">
    ...
    /*先获取组织机构树中的当前节点*/
        var sel = $('#organizationTree').tree('getSelected');
        parent.$.modalDialog({
            title : '为<' + sel.text + '>导入',
            width : 500,
            height : 300,
            href : '${path }/user/importPage?id='+sel.id,
            buttons : [ {
                text : '导入',
                handler : function() {
                    parent.$.modalDialog.openner_dataGrid = userDataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
                    var f = parent.$.modalDialog.handler.find('#userImportForm');
                    f.submit();
                }
            } ]
        });
...
<div id="userToolbar" style="display: none;">
    ...
    <shiro:hasPermission name="/common/importuser">
        <a onclick="importUserFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'fi-plus icon-green'">导入</a>
    </shiro:hasPermission>
</div>